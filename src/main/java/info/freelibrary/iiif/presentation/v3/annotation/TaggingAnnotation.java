...
public class TaggingAnnotation extends WebAnnotation implements Annotation<WebAnnotation> {
    ...
    /**
     * Creates a tagging annotation from the supplied annotation ID and target list.
     *
     * @param aID An annotation ID
     * @param aTargetList A list of annotation targets
     */
    public TaggingAnnotation(final String aID, final List<Target> aTargetList) {
    ...
    /**
     * Creates a tagging annotation from the supplied annotation ID and array of targets.
     *
     * @param aID An annotation ID
     * @param aTargetArray An array of annotation targets
     */
    public TaggingAnnotation(final String aID, final Target... aTargetArray) {
    ...
    @Override
    public TaggingAnnotation setLabel(final Label aLabel) {
        return (TaggingAnnotation) super.setLabel(aLabel);
    }

    @Override
    public TaggingAnnotation setTargets(final Target... aTargetArray) {
        return (TaggingAnnotation) super.setTargets(aTargetArray);
    }

    @Override
    public TaggingAnnotation setTargets(final List<Target> aTargetList) {
        return (TaggingAnnotation) super.setTargets(aTargetList);
    }

    @Override
    public TaggingAnnotation setStylesheet(final String aStylesheet) {
        return (TaggingAnnotation) super.setStylesheet(aStylesheet);
    }

    @Override
    public TaggingAnnotation setContexts(final List<URI> aContextList) {
        return (TaggingAnnotation) super.setContexts(aContextList);
    }
    ...
    public final TaggingAnnotation setMotivation(final Motivation aMotivation) {
        if (!Purpose.TAGGING.toString().equalsIgnoreCase(aMotivation.toString())) {
            throw new IllegalArgumentException(LOGGER.getMessage(MessageCodes.JPA_038,
                    TaggingAnnotation.class.getSimpleName(), Purpose.TAGGING, aMotivation));
        }
