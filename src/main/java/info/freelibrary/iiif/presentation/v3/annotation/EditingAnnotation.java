...
public class EditingAnnotation extends WebAnnotation implements Annotation<WebAnnotation> {
    ...
    public EditingAnnotation(final Minter aMinter) {
        ...
        setMotivation(Motivation.fromLabel(Purpose.EDITING));
    }
    ...
    /**
     * Creates an editing annotation from the supplied annotation ID and a target list.
     *
     * @param aID An annotation ID
     * @param aTargetList A list of annotation targets
     */
    public EditingAnnotation(final String aID, final List<Target> aTargetList) {
    ...
    /**
     * Creates an editing annotation from the supplied annotation ID and target array.
     *
     * @param aID An annotation ID
     * @param aTargetArray An array of annotation targets
     */
    public EditingAnnotation(final String aID, final Target... aTargetArray) {
    ...
    public final EditingAnnotation setMotivation(final Motivation aMotivation) {
        if (!Purpose.EDITING.toString().equalsIgnoreCase(aMotivation.toString())) {
            throw new IllegalArgumentException(LOGGER.getMessage(MessageCodes.JPA_038,
                    EditingAnnotation.class.getSimpleName(), Purpose.EDITING, aMotivation));
        }

        return (EditingAnnotation) super.setMotivation(Motivation.fromLabel(Purpose.EDITING));
    }
}
