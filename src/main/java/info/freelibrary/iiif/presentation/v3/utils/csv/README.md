...
### Uploading to a Test Server

You can configure `jpv3` to upload to a test server directly with the `-H`, `-U`, and `-P` options. However, if you
prefer environment variables and want to avoid repeatedly changing `JPV3_HOST`, `JPV3_USERNAME`, and `JPV3_PASSWORD`,
you can define separate test-specific variables instead:
...
Then run `jpv3` with the `-t` or `--test` option. When this option is present, `jpv3` ignores `JPV3_HOST`,
`JPV3_USERNAME`, and `JPV3_PASSWORD` and uses the corresponding `TEST_JPV3_*` variables instead.
