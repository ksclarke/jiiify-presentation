...
public class CanvasAnnotationDeserializer extends StdDeserializer<Annotation<?>> {
    ...
    /**
     * Creates a new <code>CanvasAnnotationDeserializer</code>.
     */
    public CanvasAnnotationDeserializer() {
     ...
     */
    public CanvasAnnotationDeserializer(final Class<?> aClass) {
        super(aClass);
    }
    ...
    public Annotation<?> deserialize(final JsonParser aParser, final DeserializationContext aContext)
        ...
        final ThrowingBiFunction<String, JsonNode, String, InputCoercionException> check = (aKey, aNode) -> {
            // Check that required value exists in the PaintingAnnotation's JSON serialization and fail if it doesn't
            if (aNode == null) {
    ...
    private InputCoercionException error(final JsonParser aParser, final String aMessage) {
        return new InputCoercionException(aParser, aMessage, aParser.getCurrentToken(), PaintingAnnotation.class);
    }
     ...
     * @return A new painting or supplementing annotation
        ...
        if (purpose.isPresent()) {
            return switch (purpose.get()) {
                case PAINTING -> {
                    ...
                    if (styleSheet != null) {
                        anno.setStylesheet(JSON.convertValue(styleSheet, Stylesheet.class));
                    }
    ...
    private Target getTarget(final JsonNode aNode, final JsonParser aParser) throws InputCoercionException {
        if (aNode == null) {
            ...
            throw new InputCoercionException(aParser, message, aParser.getCurrentToken(), PaintingAnnotation.class);
        }
