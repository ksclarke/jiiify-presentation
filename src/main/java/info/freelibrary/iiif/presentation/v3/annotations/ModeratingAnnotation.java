...
public class ModeratingAnnotation extends WebAnnotation implements Annotation<WebAnnotation> {
    ...
    public ModeratingAnnotation(final Minter aMinter) {
        ...
        setMotivation(Motivation.from(Purpose.MODERATING));
    }
    ...
    /**
     * Creates a moderating annotation from the supplied annotation ID and target URI.
     *
     * @param aID An annotation ID
     * @param aTarget An annotation target
     */
    public ModeratingAnnotation(final String aID, final Target aTarget) {
        super(aID, aTarget);
        setMotivation(Motivation.from(Purpose.MODERATING));
    }
    ...
    @Override
    public ModeratingAnnotation setBody(final ContentResource<?>... aResourceArray) {
        return (ModeratingAnnotation) super.setBody(aResourceArray);
    }
    ...
    public final ModeratingAnnotation setMotivation(final Motivation aMotivation) {
        if (!Purpose.MODERATING.toString().equalsIgnoreCase(aMotivation.toString())) {
            throw new IllegalArgumentException(
                    LOGGER.getMessage(MessageCodes.JPA_038, ModeratingAnnotation.class.getSimpleName(), aMotivation));
        }

        return (ModeratingAnnotation) super.setMotivation(Motivation.from(Purpose.MODERATING));
    }

    @Override
    public ModeratingAnnotation setTarget(final Target aTarget) {
        return (ModeratingAnnotation) super.setTarget(aTarget);
    }

    @Override
    public ModeratingAnnotation setTimeMode(final TimeMode aTimeMode) {
        return (ModeratingAnnotation) super.setTimeMode(aTimeMode);
    }
}
