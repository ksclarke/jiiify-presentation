...
public class BookmarkingAnnotation extends WebAnnotation {
    ...
    /**
     * Creates a bookmarking annotation from the supplied annotation ID and targets list.
     *
     * @param aID An annotation ID
     * @param aTargetList A list of annotation targets
     */
    public BookmarkingAnnotation(final String aID, final List<Target> aTargetList) {
    ...
    /**
     * Creates a bookmarking annotation from the supplied annotation ID and array of targets.
     *
     * @param aID An annotation ID
     * @param aTargetArray An array of annotation targets
     */
    public BookmarkingAnnotation(final String aID, final Target... aTargetArray) {
    ...
    public final BookmarkingAnnotation setMotivation(final Motivation aMotivation) {
        if (!Purpose.BOOKMARKING.toString().equalsIgnoreCase(aMotivation.toString())) {
            throw new IllegalArgumentException(LOGGER.getMessage(MessageCodes.JPA_038,
                    BookmarkingAnnotation.class.getSimpleName(), Purpose.BOOKMARKING, aMotivation.label()));
        }
