...
public class ModeratingAnnotationTest {
    ...
    /**
     * Test method for {@link ModeratingAnnotation#ModeratingAnnotation(String, Target)}.
     */
    @Test
    public final void testModeratingAnnotationStringTarget() {
        assertTrue(new ModeratingAnnotation(ID_PATTERN + getRandom(), new Target(new Canvas(myMinter))).getID()
                .startsWith(ID_PATTERN));
    }
    ...
    public final void testSetMotivationMotivationBad() {
        assertTrue(new ModeratingAnnotation(myMinter).setMotivation(Motivation.fromLabel(Purpose.ASSESSING)).getID()
                .startsWith(ID_PATTERN));
    }
}
